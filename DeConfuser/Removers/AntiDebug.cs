                        ...
                        if (m.Body.Instructions[x].OpCode.Code == Code.Call)
                        {
                            ...
                            if (m.Body.Instructions[x].Operand.GetType() == typeof(MethodDefinition))
                            {
                                ...
                                if (method.HasBody)
                                {
                                    //lets scan signature
                                    bool found = true;
                                    for (int j = 0; j < method.Body.Instructions.Count && j < Signature.Length; j++)
                                    {
                                        if (method.Body.Instructions[j].OpCode != Signature[j])
                                        {
                                            found = false;
                                            break;
                                        }
                                    ...
                                    if (found)
                                    {
                                        AntiType = (TypeDefinition)method.DeclaringType;
                                        AntiMethod = method;
                                        return true;
                                    }
                                }
